...
try:
    ...
    # (Đảm bảo các file này đã được cập nhật để in chi tiết)
    ...
    print("  2. gaussI.py")
    print("  3. gaussII.py")
    print("  4. newton_tien_moccachdeu.py (vì gauss cần)")
    sys.exit(1)
...
def chay_vi_du_gauss():
    ...
    if X_new_2 is not None:
        ...
        if P_n_2 is not None:
            ...
            pass # Chỉ chạy để P_n_2_expanded tồn tại
    ...
    if 'X_new_1' in locals() and X_new_1 is not None:
        ...
        # Gọi hàm GAUSS II (sẽ tự in chi tiết)
        ...
        if P_n_G2 is not None:
            ...
            # So sánh
            print(f"\n  (So sánh P_Gauss_I(x) = {P_n_1_expanded})")
            
            x = sympy.symbols('x')
            P_val_G2 = P_n_G2_expanded.subs(x, x_target_1)
            print(f"  Kiểm tra: Giá trị P({x_target_1}) = {P_val_G2:.8f}")
    ...
    if 'X_new_2' in locals() and X_new_2 is not None:
        ...
        if P_n_G2_2 is not None:
            ...
            print(f"\n  (So sánh P_Gauss_I(x) = {P_n_2_expanded})")
    else:
